#!/usr/bin/env bash
# CC Notification hook (HOST install) -> bridge /internal/hooks/notification.
...
if [[ ! -r "$TOKEN_FILE" ]]; then
  logger -t cc-hook "notify-hook: token file not readable: $TOKEN_FILE" || true
  exit 0
fi
TOKEN=$(< "$TOKEN_FILE")
...
if [[ -z "$PROJECT_ID" ]]; then
  logger -t cc-hook "notify-hook: cwd missing from payload" || true
  exit 0
fi

ENRICHED=$(jq --arg p "$PROJECT_ID" '. + {projectId: $p}' <<<"$PAYLOAD")

curl -fsS --max-time 5 -X POST "$BRIDGE_URL/internal/hooks/notification" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "$ENRICHED" \
  > /dev/null \
  || logger -t cc-hook "notify-hook: POST failed for $PROJECT_ID" || true
